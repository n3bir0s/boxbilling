...
class Service
{
    ...
    public function install()
    {
        ...
        $db = $this->di['db'];
				...
				$sql = "CREATE TABLE `activity_client_sms` (
        ...
        //throw new \Box_Exception("Throw exception to terminate module installation process with a message", array(), 123);
				...
				$this->di['logger']->info('SMS Gateway notifications installed');
    ...
    /**
		 * Method is used to fetch SMS Gateways plugins
		 *
     * @return array
     */		
		public function getGatewaysPlugins(){
    ...
    private function _getPlugin(\Model_SMSGateway $model)
    {
        $plugins = $this->getLicensePlugins();
        foreach($plugins as $plugin) {
            if($model->plugin == $plugin['filename']) {
                require_once $plugin['path'];
                $class_name = 'Box\\Mod\\Smsgateway\\Plugin\\'.$model->plugin;
                return new $class_name();
            }
        }
        error_log(sprintf('SMS Gateway #%s plugin %s is not valid', $model->id, $model->plugin));
        return null;
    }		
     ...
     * @param \Box_Event $event
     */
    public static function onAfterClientCalledSmsgatewayModule(\Box_Event $event)
    {
    ...
    public static function onAfterClientOrderCreate(\Box_Event $event)
    {
        ...
        /*
         ...
         $email['code']      = 'mod_smsgateway_email'; //@see bb-modules/Example/html_email/mod_smsgateway_email.phtml
